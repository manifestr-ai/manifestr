...
export class AIController extends BaseController {
  ...
  protected initializeRoutes(): void {
    this.routes = [
      // Old Motivation Quote Endpoint
      {
        ...
        handler: this.getMotivationQuote,
      },
      ...
      // New Agentic Endpoints (Protected)
      {
        verb: "POST",
        path: "/generate",
        handler: this.startGeneration,
        middlewares: [authenticateToken],
      },
      {
        verb: "GET",
        path: "/status/:id",
        handler: this.getJobStatus,
        middlewares: [authenticateToken],
      },
      {
        verb: "GET",
        path: "/generations",
        handler: this.getUserGenerations,
        middlewares: [authenticateToken],
      },
      {
        verb: "GET",
        path: "/recent-generations",
        handler: this.getRecentGenerations,
        middlewares: [authenticateToken],
      },
      {
        verb: "GET",
        path: "/recent-activity",
        handler: this.getRecentActivity,
        middlewares: [authenticateToken],
      },
      {
        verb: "POST",
        path: "/pin/:documentId",
        handler: this.pinDocument,
        middlewares: [authenticateToken],
      },
      {
        verb: "DELETE",
        path: "/pin/:documentId",
        handler: this.unpinDocument,
        middlewares: [authenticateToken],
      },
      {
        verb: "GET",
        path: "/pinned",
        handler: this.getPinnedDocuments,
        middlewares: [authenticateToken],
      },
      {
        verb: "POST",
        path: "/archive/:documentId",
        handler: this.archiveDocument,
        middlewares: [authenticateToken],
      },
      {
        verb: "POST",
        path: "/unarchive/:documentId",
        handler: this.unarchiveDocument,
        middlewares: [authenticateToken],
      },
      {
        verb: "GET",
        path: "/archived",
        handler: this.getArchivedDocuments,
        middlewares: [authenticateToken],
      },
      {
        verb: "DELETE",
        path: "/generation/:id",
        handler: this.deleteGeneration,
        middlewares: [authenticateToken],
      },
      {
        verb: "POST",
        path: "/restore/:documentId",
        handler: this.restoreDocument,
        middlewares: [authenticateToken],
      },
      {
        verb: "GET",
        path: "/deleted",
        handler: this.getDeletedDocuments,
        middlewares: [authenticateToken],
      },
      {
        verb: "GET",
        path: "/generation/:id",
        handler: this.getGenerationDetails,
        middlewares: [optionalAuth],
      },
      {
        verb: "PATCH",
        path: "/generation/:id",
        handler: this.updateGeneration,
        middlewares: [optionalAuth],
      },
      {
        verb: "GET",
        path: "/processing-generations",
        handler: this.getUserProcessingGenerations,
        middlewares: [authenticateToken],
      },
    ];
  }
  ...
  private updateGeneration = async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!updated) {
        ...
        const collaboration =
          collaborators && collaborators.length > 0 ? collaborators[0] : null;
  ...
  private startGeneration = async (req: AuthRequest, res: Response) => {
    try {
      ...
      try {
        const job = await this.orchestrator.startGeneration(
        ...
        return res.json({
          ...
          data: {
            ...
            status: "pending",
            warning:
              "Job created but not queued for processing. Check AWS credentials.",
          },
        });
      }
    } catch (error) {
  ...
  private getUserProcessingGenerations = async (
    req: AuthRequest,
    res: Response,
  ) => {
    try {
      ...
      const jobs = await SupabaseDB.getProcessingGenerationJobs(userId);
      ...
      const docsWithLastAccess = await Promise.all(
        allDocs.map(async (doc) => {
          ...
          const lastActivityTime =
            session?.last_seen || doc.updated_at || doc.created_at;
    ...
    try {
      ...
      if (!job) {
        ...
        if (!collab) {
          return res
            .status(403)
            .json({ status: "error", message: "No access to this document" });
        }
      }
      ...
      if ((count || 0) >= 10) {
        return res.status(400).json({
          status: "error",
          message:
            "Maximum 10 pinned documents reached. Unpin a document first.",
        });
      }

      // Pin the document
      const { error } = await supabaseAdmin.from("pinned_documents").insert({
        user_id: userId,
      ...
      if (error) {
        if (error.code === "23505") {
          return res
            .status(400)
            .json({ status: "error", message: "Document already pinned" });
        }
        throw error;
      }
  ...
  private deleteGeneration = async (req: AuthRequest, res: Response) => {
    try {
      ...
      console.log(
        `🗑️ User ${userId} soft deleted document ${id} (status: DELETED, updated_at as deletion time)`,
      );
  ...
  private getGenerationDetails = async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!job) {
        console.log(
          `📊 User ${userId} is not owner, checking collaboration access...`,
        );
        ...
        console.log(`🔍 Collaboration check result:`, {
          collaboration,
          collabError,
        });
        ...
        if (collaboration) {
          console.log(
            ` User is collaborator (${collaboration.role}), allowing access`,
          );
  ...
  private getMotivationQuote = async (req: Request, res: Response) => {
    try {
        ...
        messages: [
          {
            role: "user",
            content:
              "Generate a daily motivation quote for a professional dashboard.",
          },
        ],
      });
      ...
      const content =
        textBlock?.text?.trim() || "Turn pressure into output. Execute.";
