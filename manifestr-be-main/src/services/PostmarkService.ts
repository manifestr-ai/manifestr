...
export interface CollabProjectInviteData {
    inviteeEmail: string;
    inviteeName: string;
    inviterName: string;
    projectName: string;
...
export class PostmarkService {
    ...
    constructor() {
        ...
        this.fromEmail = process.env.POSTMARK_FROM_EMAIL || 'leah.obrien@manifestr.ai';
    ...
    /**
     * Send collab project invite email
     */
    async sendCollabProjectInvite(data: CollabProjectInviteData): Promise<boolean> {
        try {
            const htmlBody = this.buildCollabProjectInviteHTML(data);
            const textBody = this.buildCollabProjectInviteText(data);

            console.log(`📧 Sending collab project invite to ${data.inviteeEmail}...`);
            ...
            console.log(`✅ Collab project invite email sent! Message ID: ${response.MessageID}`);
            return true;

        } catch (error: any) {
            console.error('❌ Failed to send collab project invite:', error);
...
Questions? Contact us at support@manifestr.ai`;
    ...
    /**
     * Build HTML email for collab project invite
     */
    private buildCollabProjectInviteHTML(data: CollabProjectInviteData): string {
        const { inviteeEmail, inviteeName, inviterName, projectName, role, inviteLink, accountCreated, defaultPassword } = data;
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Collab Project Invite</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f5f5f5; padding: 40px 0;">
        <tr>
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                    ...
                    <tr>
                        <td style="padding: 40px;">
                            ...
                            <p style="margin: 0 0 20px; font-size: 16px; line-height: 24px; color: #333333;">
                                <strong>${inviterName}</strong> has invited you to join their collaboration project:
                            </p>
                            ...
                            <p style="margin: 0 0 10px; font-size: 14px; line-height: 20px; color: #666666;">
                                ${role === 'admin' ? 'You can manage members and documents in this project.' : role === 'editor' ? 'You can add and edit documents in this project.' : 'You can view all documents in this project.'}
                            </p>
    ...
    /**
     * Build plain text email for collab project invite
     */
    private buildCollabProjectInviteText(data: CollabProjectInviteData): string {
        const { inviteeName, inviterName, projectName, role, inviteLink, accountCreated, inviteeEmail, defaultPassword } = data;

        const roleDescription = role === 'admin' 
            ? 'You can manage members and documents in this project.'
            : role === 'editor' 
            ? 'You can add and edit documents in this project.'
            : 'You can view all documents in this project.';

        let text = `Hi ${inviteeName || 'there'},

${inviterName} has invited you to join their collaboration project: "${projectName}"
