...
export default function FormatPanel({ store }: FormatPanelProps) {
  ...
  // Simple placeholder handlers - these can be implemented later
  const handleBorders = () => {
    alert("Borders: Right-click on cells and select 'Borders' to add cell borders.");
  };

  const handleFills = () => {
    alert("Fill Color: Use the toolbar's fill color button or right-click on cells to change background color.");
  };

  const handleFormat = () => {
    alert("Number Format: Use the toolbar's number format dropdown to format numbers, dates, currency, etc.");
  };

  const handleFreezeRows = () => {
    alert("Freeze Rows: Select a row, then use View > Freeze Rows from the menu.");
  };

  const handleFreezeCols = () => {
    alert("Freeze Columns: Select a column, then use View > Freeze Columns from the menu.");
  };

  const handleMerge = () => {
    alert("Merge Cells: Select multiple cells, then right-click and choose 'Merge Cells'.");
  };

  const handleSplit = () => {
    alert("Split/Unmerge Cells: Select merged cells, then right-click and choose 'Unmerge Cells'.");
  };

  const handleWrap = () => {
    alert("Text Wrap: Select cells, then right-click and choose 'Wrap Text' to enable text wrapping.");
  };

  const handleAlign = () => {
    alert("Text Alignment: Use the HOME tab's alignment buttons (Left, Center, Right) to align cell content.");
  };

  const handleRotate = () => {
    alert("Text Rotation: Select cells, then right-click and choose 'Format Cells' to rotate text.");
  };

  const handleRules = () => {
    alert("Conditional Formatting: This feature will allow you to highlight cells based on their values. Coming soon!");
  };
