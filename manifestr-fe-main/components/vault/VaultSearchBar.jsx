...
export default function VaultSearchBar({
  ...
  const handleResetFilters = () => {
    ...
    setSelectedSort("Last Edited")
    setSelectedCollab("All Collabs")
  ...
  const notifyFilters = (updates = {}) => {
    const newFilters = {
      tool: updates.tool ?? selectedTool,
      sort: updates.sort ?? selectedSort,
      collab: updates.collab ?? selectedCollab,
    }
  ...
  return (
    <div className="px-4 md:px-[30px] py-6 space-y-4">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-end gap-4">
        ...
        <div className={`flex flex-wrap items-center gap-3 w-full md:w-auto ${leftSide ? 'md:mr-auto' : 'md:ml-auto'}`}>
                  ...
                  {tools.map((tool) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className={tool !== "All tools" ? "ml-[28px]" : ""}
                        >
                          {tool}
                        </span>
                      </div>
                  ...
                  {collabOptions.map((option) => (
                    <motion.button
                      ...
                      onClick={() => {
                        ...
                        setShowMoreFiltersDropdown(false)
                        notifyFilters({ collab: option })
                      }}
                      whileHover={{ backgroundColor: "#f4f4f5" }}
                      className="w-full px-3 py-2 text-left text-[14px] leading-[21px] text-[#18181b] flex items-center justify-between gap-3"
                    >
                      <span>{option}</span>
                      {selectedCollab === option && (
                        <Check className="w-4 h-4 text-[#18181b]" />
                      )}
                    </motion.button>
                  ))}
        ...
        <div className="flex items-center gap-4 self-end md:self-auto">
          <motion.button
            ...
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="text-[#71717B] font-inter text-[13px] font-medium leading-[19.5px] tracking-[-0.076px] transition-colors hover:text-[#18181b]"
          >
            Reset Filters
          </motion.button>
